...
in_node = 12
model = Sequential()
model.add(Dense(128, input_shape = (in_node, ), activation='tanh'))
...
target_model.add(Dense(128, input_shape = (in_node, ), activation='tanh'))
...
for i in range(num_episodes):
	...
	while not done:
		...
		if np.random.random() < eps:
			# essendo che devo passare una array di azioni, e' sufficiente passarne uno
			# formato solo dall'azione casuale scelta
			# NOTA: passo un array di azioni perché l'env, se vede che l'azione migliore
			# porta a collisione, allora sceglie la seconda, e via dicendo
			a = np.zeros(in_node, dtype=int) + np.random.randint(0, out_node)
		else:
			# Altrimenti, la prossima azione e' scelta utilizzando l'ANN per prevedere la mossa migliore.
			# Le ordino secndo questa metrica, ::-1 serve a fare decrescente
			a = np.argsort(model.predict(np.array([s]))[0])[::-1]
		...
		# ricevo inoltre l'azione realmente eseguita e me la salvo, la appendero' dopo
		new_s, r, done, moves, real_a = env_1.step(a)
		...
		if(r > 0):
			memory_win.append([s, real_a, r, new_s, done])
		elif(r < 0):
			memory_lose.append([s, real_a, r, new_s, done])
		else:
			memory.append([s, real_a, r, new_s, done])
