...
def reach(target_angles, joint_states, obstacle, distance_check):
	...
	# rospy sleep
	rate.sleep()
	# ...and do the same until we're done or timeouted
...
def talker():
	...
	for sample in range (episodes):
		...
		# parameter for the stuckness-detection
		stuckness_tolerance = 14
		# data structure which keeps track of the last positions
		position_tracker = deque(maxlen = stuckness_tolerance)
		position_tracker.append([x,y,z])
		...
		for iter in range(150):
			...
			# this list contains the best actions: 0 is best, 9 is worst.
			actions = np.argsort(model.predict(np.array([input_layer]))[0])[::-1]
			# starting with the best action, we check if with this we would hit. If so, then, we try next one.
			# if everything collides, then, we still use the best one. We can't do anything else.			
			action = actions[0]
			for i in range(len(actions)):
				if isItSafe(state_deg, actions[i], obstacle, distance_check, step = 10):
					action = actions[i]
					break

			# if we have enough actions memorized...
			if len(position_tracker) >= stuckness_tolerance:
				# take the oldest action and the new, check if they're too close
				pos_old = position_tracker.popleft()
				pos_new = [x, y, z]
				if math.sqrt( math.pow((pos_old[0]-pos_new[0]), 2) + math.pow((pos_old[1]-pos_new[1]), 2) + math.pow((pos_old[2]-pos_new[2]), 2) ) <= 0.30:
					# in that case, we're stuck. let's try a random action.
					action = np.random.randint(0, 10)
					while not isItSafe(state_deg, action, obstacle, distance_check):
						action = np.random.randint(0, 10)
					# we restart the queue.
					position_tracker = deque(maxlen = stuckness_tolerance)
					# we save 90% of the "stuck" movments...
					iter -= stuckness_tolerance * 0.9
					print("####### unstuck! ######")
