...
# genera un ostacolo casuale tra il braccio e il target
	...
	def compute_new_state(self, state, action):
		# solo se fattibile
		if ( (action%2) == 0 and (state[int(action/2)] + self.step_size ) < self.maxRange[int(action/2)] ):
			state[int(action/2)] += self.step_size

		if ( (action%2) == 1 and (state[int(action/2)] - self.step_size ) > self.minRange[int(action/2)] ):
			state[int(action/2)] -= self.step_size

		return state

		# given a certain state, a certain action, a certain obstacle, is it safe? with a certain step, also!
	def isItSafe(self, joint_states, action):
		return not self.hasHitObstacle(self.vectGetJointPos(self.compute_new_state(joint_states, action)))
	...
	# original_vect is used in case, with the "fastest way", we would have a collision. See after
		...
		for i in range(len(self.target_angle)):
			if (self.target_angle[i] - self.state[i]) > self.step_size:
				self.state[i] += self.step_size
				done = False
			elif (self.target_angle[i] - self.state[i]) < -self.step_size:
				self.state[i] -= self.step_size
				done = False
			else:
	...
	def step(self, action):
		...
		return np.concatenate((self.normalizeState(), self.target, self.obstacle, self.obstacle_radius )), reward, done, self.counter
